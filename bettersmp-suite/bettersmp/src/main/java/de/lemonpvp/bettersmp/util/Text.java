...
/**
 * MiniMessage-Helfer: versteht MiniMessage-Tags UND Legacy-Farbcodes
 * (&7, &#FFD75A, &x&F&F..., Paragraph-Codes) in derselben Zeichenkette.
 */
public final class Text {
    ...
    public static String legacyToMini(String s) {
        ...
        Matcher x = HEX_X.matcher(s);
        if (x.find()) {
            StringBuilder sb = new StringBuilder();
            x.reset();
            while (x.find()) {
                String hex = x.group().replaceAll("[&§x]", "");
                x.appendReplacement(sb, "<#" + hex + ">");
            }
            x.appendTail(sb);
            s = sb.toString();
        }
        s = HEX.matcher(s).replaceAll(mr -> "<#" + mr.group(1) + ">");
