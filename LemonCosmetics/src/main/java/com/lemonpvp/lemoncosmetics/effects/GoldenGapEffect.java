    ...
    public static void play(LemonCosmetics plugin, Location loc) {
        ...
        task[0] = plugin.getServer().getScheduler().runTaskTimerAsynchronously(plugin, () -> {
            ...
            for (int i = 0; i < 6; i++) {
                ...
                loc.getWorld().spawnParticle(Particle.GLOW, loc.clone().add(x, y, z), 1, 0.05, 0.05, 0.05, 0);
                loc.getWorld().spawnParticle(Particle.FALLING_DUST, loc.clone().add(x, y + 0.1, z), 1, 0, 0, 0, 0, goldData);
            }
            tick[0]++;
        }, 0L, 1L);
    }
