...
public enum PlankTier {
    ...
    OAK      ("Eiche",         0,          -1, 10, Material.OAK_LOG),
    BIRCH    ("Birke",         50_000,     30, 15, Material.BIRCH_LOG),
    SPRUCE   ("Fichte",       125_000,     30, 20, Material.SPRUCE_LOG),
    ACACIA   ("Akazie",       175_000,     30, 25, Material.ACACIA_LOG),
    DARK_OAK ("Dunkeleiche",  200_000,     30, 30, Material.DARK_OAK_LOG),
    JUNGLE   ("Dschungel",    300_000,     30, 40, Material.JUNGLE_LOG);
    ...
    public String priceDisplay() {
        if (isFree()) return "<green>Kostenlos";
        return "<yellow>" + com.lemonpvp.lemonlobby.util.FormatUtil.formatAmount(appleCost) + " <white>Äpfel";
    }

    public String durationDisplay() {
        if (durationDays < 0) return "<green>Dauerhaft";
        return "<yellow>" + durationDays + " Tage";
    }
